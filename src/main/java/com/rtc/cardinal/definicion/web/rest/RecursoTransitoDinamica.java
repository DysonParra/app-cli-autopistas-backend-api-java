/*
 * @fileoverview {RecursoTransitoDinamica} se encarga de realizar tareas especificas.
 *
 * @version             1.0
 *
 ...
 *
 * @History v1.0 --- La implementacion de {RecursoTransitoDinamica} fue realizada el 31/07/2022.
 * @Dev - La primera version de {RecursoTransitoDinamica} fue escrita por Dyson A. Parra T.
 */
...
import com.rtc.cardinal.definicion.web.rest.ensamblador.EnsambladorRecursoTransitoDinamica;
...
@Api(value = "TransitoDinamica API", description = "Gestión de TransitoDinamica")
public class RecursoTransitoDinamica {

    private final Logger log = LoggerFactory.getLogger(RecursoTransitoDinamica.class);
    private final ServicioTransitoDinamica servicioEntidad;
    private final EnsambladorRecursoTransitoDinamica ensambladorRecursoEntidad;

    public RecursoTransitoDinamica(ServicioTransitoDinamica servicioEntidad,
            EnsambladorRecursoTransitoDinamica ensambladorRecursoEntidad) {
    ...
    @GetMapping("/TransitoDinamica/paginas")
    public ResponseEntity<List<TransitoDinamicaDTO>> obtenerEntidades(Pageable pageable) {
        log.debug("REST request to get a page of TransitoDinamica");
        ...
        HttpHeaders headers = PaginationUtil.generatePaginationHttpHeaders(page, "/api/TransitoDinamica/paginas");
    ...
    @GetMapping("/TransitoDinamica/{expresion}/paginas")
    public ResponseEntity<List<TransitoDinamicaDTO>> obtenerEntidades(@PathVariable String expresion, Pageable pageable) {
        log.debug("REST request to get a page of TransitoDinamica");
        Page<TransitoDinamicaDTO> page = null;
        try {
        ...
        HttpHeaders headers = PaginationUtil.generatePaginationHttpHeaders(page, "/api/TransitoDinamica/{expresion}/paginas/" + expresion);
        return new ResponseEntity<>(page.getContent(), headers, HttpStatus.OK);
    }
    ...
    public CollectionModel<EntityModel<TransitoDinamicaDTO>> obtenerEntidades() {

        List<EntityModel<TransitoDinamicaDTO>> entidades = null;

        try {
            entidades = this.servicioEntidad.obtenerEntidades().parallelStream()
                    .map(ensambladorRecursoEntidad::toModel)
                    .collect(Collectors.toList());
            CollectionModel<EntityModel<TransitoDinamicaDTO>> recursoRetorno = new CollectionModel<>(entidades);
            recursoRetorno.add(linkTo(methodOn(RecursoTransitoDinamica.class).obtenerEntidades()).withSelfRel());
    ...
    @GetMapping("/TransitoDinamica/requisito/{id}")
    public CollectionModel<EntityModel<TransitoDinamicaDTO>> obtenerEntidades(@PathVariable String id) {
    ...
    public ResponseEntity<EntityModel<TransitoDinamicaDTO>> buscarEntidad(@PathVariable String id) {

        log.debug(String.format("servicio-transitoDinamica buscarEntidad() invocado:{} por {} ",
    ...
    public ResponseEntity<?> guardarEntidad(@RequestBody TransitoDinamicaDTO entidadDTO) {
    ...
    public ResponseEntity<?> actualizarEntidad(@RequestBody TransitoDinamicaDTO entidadDTO, @PathVariable Long id) {

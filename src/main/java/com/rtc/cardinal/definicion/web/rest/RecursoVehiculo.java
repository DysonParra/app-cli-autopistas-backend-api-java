/*
 * @fileoverview {RecursoVehiculo} se encarga de realizar tareas especificas.
 *
 * @version             1.0
 *
 ...
 *
 * @History v1.0 --- La implementacion de {RecursoVehiculo} fue realizada el 31/07/2022.
 * @Dev - La primera version de {RecursoVehiculo} fue escrita por Dyson A. Parra T.
 */
...
import com.rtc.cardinal.definicion.web.rest.ensamblador.EnsambladorRecursoVehiculo;
...
/**
 * TODO: Definición de {@code RecursoVehiculo}.
 *
 * @author Dyson Parra
 * @since 1.8
 */
...
@Api(value = "Vehiculo API", description = "Gestión de Vehiculo")
public class RecursoVehiculo {

    private final Logger log = LoggerFactory.getLogger(RecursoVehiculo.class);
    private final ServicioVehiculo servicioEntidad;
    private final EnsambladorRecursoVehiculo ensambladorRecursoEntidad;

    public RecursoVehiculo(ServicioVehiculo servicioEntidad,
            EnsambladorRecursoVehiculo ensambladorRecursoEntidad) {
    ...
    @GetMapping("/Vehiculo/paginas")
    public ResponseEntity<List<VehiculoDTO>> obtenerEntidades(Pageable pageable) {
        log.debug("REST request to get a page of Vehiculo");
        ...
        HttpHeaders headers = PaginationUtil.generatePaginationHttpHeaders(page, "/api/Vehiculo/paginas");
    ...
    @GetMapping("/Vehiculo/{expresion}/paginas")
    public ResponseEntity<List<VehiculoDTO>> obtenerEntidades(@PathVariable String expresion, Pageable pageable) {
        log.debug("REST request to get a page of Vehiculo");
        Page<VehiculoDTO> page = null;
        try {
        ...
        HttpHeaders headers = PaginationUtil.generatePaginationHttpHeaders(page, "/api/Vehiculo/{expresion}/paginas/" + expresion);
        return new ResponseEntity<>(page.getContent(), headers, HttpStatus.OK);
    }
    ...
    @GetMapping("/Vehiculo")
    public CollectionModel<EntityModel<VehiculoDTO>> obtenerEntidades() {

        List<EntityModel<VehiculoDTO>> entidades = null;

        try {
            entidades = this.servicioEntidad.obtenerEntidades().parallelStream()
                    .map(ensambladorRecursoEntidad::toModel)
                    .collect(Collectors.toList());
            CollectionModel<EntityModel<VehiculoDTO>> recursoRetorno = new CollectionModel<>(entidades);
            recursoRetorno.add(linkTo(methodOn(RecursoVehiculo.class).obtenerEntidades()).withSelfRel());
    ...
    @GetMapping("/Vehiculo/requisito/{id}")
    public CollectionModel<EntityModel<VehiculoDTO>> obtenerEntidades(@PathVariable String id) {
    ...
    public ResponseEntity<EntityModel<VehiculoDTO>> buscarEntidad(@PathVariable String id) {

        log.debug(String.format("servicio-vehiculo buscarEntidad() invocado:{} por {} ",
    ...
    public ResponseEntity<?> guardarEntidad(@RequestBody VehiculoDTO entidadDTO) {
    ...
    public ResponseEntity<?> actualizarEntidad(@RequestBody VehiculoDTO entidadDTO, @PathVariable String id) {
